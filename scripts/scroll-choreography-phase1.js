(function () {
  ...
  function initialiseChoreography() {
    ...
    const heroFocusCommit = { raf: null, lastTotal: NaN, lastBase: NaN, lastPointer: NaN };
    ...
    const applyHeroFocus = () => {
      if (!heroFocusCommit.raf) {
        heroFocusCommit.raf = requestAnimationFrame(commitHeroFocus);
      }
    };
    ...
    const applyMoireState = () => {
      if (!moireCommit.raf) {
        moireCommit.raf = requestAnimationFrame(commitMoireState);
      }
    };
